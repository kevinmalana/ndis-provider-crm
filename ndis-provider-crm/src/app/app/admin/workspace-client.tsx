...
function Readiness({
  ...
  const [registrationGroup, setRegistrationGroup] = useState("synthetic");
  const [supportClass, setSupportClass] = useState("individual");
  ...
  const [verificationSource, setVerificationSource] = useState("Provider screening register");
  const [verificationReviewer, setVerificationReviewer] = useState("");
  const [interimBar, setInterimBar] = useState(false);
  const [screeningSuspension, setScreeningSuspension] = useState(false);
  const [screeningExclusion, setScreeningExclusion] = useState(false);
  const [screeningRevocation, setScreeningRevocation] = useState(false);
  ...
  const [requirementState, setRequirementState] = useState("required");
  const [assessmentMethod, setAssessmentMethod] = useState("provider_assessed");
  ...
  const [evidenceState, setEvidenceState] = useState("met");
  const [evidenceLimitation, setEvidenceLimitation] = useState("");
  const [evidenceExpiresAt, setEvidenceExpiresAt] = useState(() => new Date(Date.now() + 31536000000).toISOString().slice(0, 16));
  const [roleDefinitionBasis, setRoleDefinitionBasis] = useState("");
  const [roleDescription, setRoleDescription] = useState("");
  const [roleAssessor, setRoleAssessor] = useState("");
  const [roleAssessorTitle, setRoleAssessorTitle] = useState("");
  const [riskAssessed, setRiskAssessed] = useState(true);
  const [riskAssessedAt, setRiskAssessedAt] = useState(() => new Date().toISOString().slice(0, 16));
  ...
  const [lifecycleContextId, setLifecycleContextId] = useState("");
  const [readinessContextId, setReadinessContextId] = useState("");
  const [contextState, setContextState] = useState("active");
  const [lifecycleReviewer, setLifecycleReviewer] = useState("");
  const [lifecycleRole, setLifecycleRole] = useState("");
  const [lifecycleJurisdiction, setLifecycleJurisdiction] = useState("");
  ...
  const [readinessFingerprint, setReadinessFingerprint] = useState<string | null>(null);
  ...
  const workerLabel = (membershipId: string) => String(workers.find((entry) => String(entry.membership_id) === membershipId)?.full_name ?? "Worker record unavailable");
  const roleLabel = (roleId: string) => String(data.roles?.find((entry) => String(entry.id) === roleId)?.title ?? "Role record unavailable");
  const requirementLabel = (requirementId: string) => String(data.competenceRequirements?.find((entry) => String(entry.id) === requirementId)?.evidence_type ?? "Requirement unavailable");
  const selectedShift = data.shifts.find((s) => String(s.id) === shift);
  const selectedLifecycleContext = data.serviceContexts?.find((c) => String(c.id) === lifecycleContextId);
  const selectedReadinessContext = data.serviceContexts?.find((c) => String(c.id) === readinessContextId);
  ...
  async function checkReadiness() {
    ...
    const checkedFingerprint = currentReadinessFingerprint;
    const participantId = String(selectedReadinessContext?.participant_id ?? "");
    const { data: result, error } = await readRpc("list_admin_provider_readiness", { p_organisation_id: organisationId, p_worker_membership_id: worker, p_participant_id: participantId, p_context_id: readinessContextId, p_scheduled_start: new Date(scheduledStart).toISOString(), p_scheduled_end: new Date(scheduledEnd).toISOString() });
    if (error) { setReadinessResult(null); setReadinessFingerprint(null); setReadinessError(`${error.message}${error.details ? ` — ${error.details}` : ""}`); }
    else { setReadinessResult((result ?? {}) as Record<string, unknown>); setReadinessFingerprint(checkedFingerprint); }
    setReadinessPending(false);
  }
  function chooseLifecycleContext(contextId: string) {
    const selected = data.serviceContexts?.find((entry) => String(entry.id) === contextId);
    setLifecycleContextId(contextId);
    setContextState(String(selected?.lifecycle_state ?? "draft"));
    setLifecycleReviewer(String(selected?.reviewer_profile_id ?? ""));
    setLifecycleRole(String(selected?.role_version_id ?? ""));
    setLifecycleJurisdiction(String(selected?.jurisdiction ?? ""));
  }
  return (
    <div className="space-y-6">
      <Card>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card><CardHeader><CardTitle>1–2. Provider scope and capability</CardTitle><CardDescription>Versioned jurisdiction and explicit support boundary.</CardDescription></CardHeader><CardContent className="space-y-4">
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); void call(FORM_KEYS.providerScope, "cmd_admin_create_provider_scope_version", { p_organisation_id: organisationId, p_registration_state: registrationState, p_registration_group: registrationGroup, p_class_of_support: supportClass, p_jurisdictions: jurisdictions.split(",").map((x) => x.trim()).filter(Boolean), p_effective_from: effectiveFrom, p_effective_until: effectiveUntil, p_reviewed_by: scopeReviewer || null, p_payload: { source: "admin-readiness" } }); }}>
            ...
            <Field label="Registration group"><Input required value={registrationGroup} onChange={(e) => setRegistrationGroup(e.target.value)} /></Field><Field label="Class of support"><Input required value={supportClass} onChange={(e) => setSupportClass(e.target.value)} /></Field><Field label="Scope reviewer"><select required className={inputBase} value={scopeReviewer} onChange={(e) => setScopeReviewer(e.target.value)}><option value="">Choose admin reviewer</option>{officePeople.filter((person) => person.role === "admin").map((person) => <option key={String(person.profile_id)} value={String(person.profile_id)}>{identityLabel(String(person.profile_id))}</option>)}</select></Field>
            ...
            <Button type="submit" disabled={actorRole !== "admin" || isPending(FORM_KEYS.providerScope)} aria-busy={isPending(FORM_KEYS.providerScope)}>Save provider scope version</Button>{actorRole !== "admin" ? <p className="text-xs text-muted-foreground">Admin role required.</p> : null}{message(FORM_KEYS.providerScope)}
          </form>
          <form className="space-y-3 border-t pt-4" onSubmit={(e) => { e.preventDefault(); void call(FORM_KEYS.capability, "cmd_admin_create_support_capability", { p_organisation_id: organisationId, p_scope_version_id: scope || String(selectedScope?.id ?? ""), p_support_category: supportCategory, p_service_kind: serviceKind, p_capability: capabilityState, p_effective_from: effectiveFrom, p_effective_until: effectiveUntil, p_payload: { source: "admin-readiness" } }); }}>
            ...
            <Button type="submit" disabled={actorRole !== "admin" || isPending(FORM_KEYS.capability)} aria-busy={isPending(FORM_KEYS.capability)}>Add individual time capability</Button>{message(FORM_KEYS.capability)}
          </form>
        </CardContent></Card>
        <Card><CardHeader><CardTitle>3. Provider catalogue</CardTitle><CardDescription>Provider-managed source label/version only; no live catalogue verification.</CardDescription></CardHeader><CardContent><form className="space-y-3" onSubmit={(e) => { e.preventDefault(); void call(FORM_KEYS.catalogue, "cmd_admin_create_catalogue_item", { p_organisation_id: organisationId, p_source_label: catalogueSource, p_source_version: catalogueVersion, p_catalogue_effective_from: effectiveFrom, p_catalogue_effective_until: effectiveUntil, p_item_code: itemCode, p_item_name: itemName, p_support_category: supportCategory, p_time_unit: timeUnit, p_service_kind: serviceKind, p_item_effective_from: effectiveFrom, p_item_effective_until: effectiveUntil, p_payload: { source: "admin-readiness" } }); }}>
            ...
            <Button type="submit" disabled={actorRole !== "admin" || isPending(FORM_KEYS.catalogue)} aria-busy={isPending(FORM_KEYS.catalogue)}>Add time-based supported item</Button>{message(FORM_KEYS.catalogue)}
        ...
        <Card><CardHeader><CardTitle>4–5. Role, screening and competence evidence</CardTitle><CardDescription>Named provider records. Missing, expired, barred, suspended, excluded, revoked or incomplete evidence remains unready.</CardDescription></CardHeader><CardContent className="space-y-4">
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); void call(FORM_KEYS.workerVerification, "cmd_admin_record_worker_verification", { p_organisation_id: organisationId, p_worker_membership_id: worker, p_role_version_id: role, p_source_checked: verificationSource, p_verifier_name: verificationReviewer, p_verified_at: effectiveFrom, p_application_or_check_reference: verificationRef, p_clearance_status: screeningStatus, p_clearance_expires_at: effectiveUntil, p_interim_bar: interimBar, p_suspension: screeningSuspension, p_exclusion: screeningExclusion, p_revocation: screeningRevocation, p_effective_from: effectiveFrom, p_effective_until: effectiveUntil, p_payload: { source: "admin-readiness" } }); }}>
            ...
            <Field label="Screening source"><Input required value={verificationSource} onChange={(e) => setVerificationSource(e.target.value)} /></Field><Field label="Screening verifier"><Input required value={verificationReviewer} onChange={(e) => setVerificationReviewer(e.target.value)} /></Field><Field label="Application/check reference"><Input required value={verificationRef} onChange={(e) => setVerificationRef(e.target.value)} placeholder="SYN-CHECK-001" /></Field><Field label="Clearance status"><select className={inputBase} value={screeningStatus} onChange={(e) => setScreeningStatus(e.target.value)}><option value="current">Current</option><option value="pending">Pending</option><option value="expired">Expired</option><option value="not_required">Not required</option></select></Field><div className="grid grid-cols-2 gap-2 text-sm">{[["Interim bar",interimBar,setInterimBar],["Suspension",screeningSuspension,setScreeningSuspension],["Exclusion",screeningExclusion,setScreeningExclusion],["Revocation",screeningRevocation,setScreeningRevocation]].map(([label,value,setter]) => <label key={String(label)} className="flex gap-2"><input type="checkbox" checked={Boolean(value)} onChange={(e) => (setter as (value: boolean) => void)(e.target.checked)} />{String(label)}</label>)}</div>
          ...
          <form className="space-y-3 border-t pt-4" onSubmit={(e) => { e.preventDefault(); void call(FORM_KEYS.competenceEvidence, "cmd_admin_record_competence_evidence", { p_organisation_id: organisationId, p_worker_membership_id: worker, p_requirement_id: requirement, p_evidence_type: String(data.competenceRequirements?.find((entry) => String(entry.id) === requirement)?.evidence_type ?? requirementEvidenceType), p_issuer: evidenceIssuer, p_evidence_reference: evidenceRef, p_verifier_name: evidenceVerifier, p_assessed_state: evidenceState, p_limitation: evidenceLimitation, p_expires_at: evidenceExpiresAt ? new Date(evidenceExpiresAt).toISOString() : null, p_effective_from: effectiveFrom, p_effective_until: effectiveUntil, p_payload: { source: "admin-readiness" } }); }}>
            <Field label="Required competence"><select required className={inputBase} value={requirement} onChange={(e) => setRequirement(e.target.value)}><option value="">Choose requirement</option>{(data.competenceRequirements ?? []).map((entry) => <option key={String(entry.id)} value={String(entry.id)}>{String(entry.evidence_type)} · {String(entry.support_category)} · {String(entry.requirement_state)}</option>)}</select></Field><Field label="Evidence issuer"><Input required value={evidenceIssuer} onChange={(e) => setEvidenceIssuer(e.target.value)} /></Field><Field label="Evidence verifier"><Input required value={evidenceVerifier} onChange={(e) => setEvidenceVerifier(e.target.value)} /></Field><Field label="Evidence state"><select className={inputBase} value={evidenceState} onChange={(e) => setEvidenceState(e.target.value)}><option value="met">Met</option><option value="not_met">Not met</option><option value="pending">Pending</option></select></Field><Field label="Evidence limitation"><Input value={evidenceLimitation} onChange={(e) => setEvidenceLimitation(e.target.value)} /></Field><Field label="Evidence reference"><Input required value={evidenceRef} onChange={(e) => setEvidenceRef(e.target.value)} placeholder="SYN-COMP-001" /></Field><Field label="Evidence expiry"><Input type="datetime-local" value={evidenceExpiresAt} onChange={(e) => setEvidenceExpiresAt(e.target.value)} /></Field>
          ...
          <form className="space-y-3 border-t pt-4" onSubmit={(e) => { e.preventDefault(); void call(FORM_KEYS.riskRole, "cmd_admin_create_risk_role", { p_organisation_id: organisationId, p_title: roleTitle, p_definition_basis: roleDefinitionBasis, p_description: roleDescription, p_assessed_at: new Date(riskAssessedAt).toISOString(), p_assessor_name: roleAssessor, p_assessor_title: roleAssessorTitle, p_risk_assessed: riskAssessed, p_effective_from: effectiveFrom, p_effective_until: effectiveUntil, p_payload: { source: "admin-readiness" } }); }}>
            ...
            <Field label="Role definition basis"><Input required value={roleDefinitionBasis} onChange={(e) => setRoleDefinitionBasis(e.target.value)} /></Field><Field label="Role description"><Input required value={roleDescription} onChange={(e) => setRoleDescription(e.target.value)} /></Field><Field label="Role assessor"><Input required value={roleAssessor} onChange={(e) => setRoleAssessor(e.target.value)} /></Field><Field label="Assessor title"><Input required value={roleAssessorTitle} onChange={(e) => setRoleAssessorTitle(e.target.value)} /></Field><Field label="Assessment date"><Input required type="datetime-local" value={riskAssessedAt} onChange={(e) => setRiskAssessedAt(e.target.value)} /></Field><label className="flex gap-2 text-sm"><input type="checkbox" checked={riskAssessed} onChange={(e) => setRiskAssessed(e.target.checked)} />Risk assessed</label><Button type="submit" disabled={actorRole !== "admin" || isPending(FORM_KEYS.riskRole)} aria-busy={isPending(FORM_KEYS.riskRole)} variant="outline">Define risk-assessed role</Button>{message(FORM_KEYS.riskRole)}
          </form>
          <form className="space-y-3 border-t pt-4" onSubmit={(e) => { e.preventDefault(); void call(FORM_KEYS.screeningPolicy, "cmd_admin_create_screening_policy", { p_organisation_id: organisationId, p_role_version_id: role, p_registration_state: registrationState, p_decision: screeningDecision, p_decision_owner: policyOwner, p_decision_reason: policyReason, p_effective_from: effectiveFrom, p_effective_until: effectiveUntil, p_payload: { source: "admin-readiness" } }); }}>
            <Field label="Screening decision"><select className={inputBase} value={screeningDecision} onChange={(e) => setScreeningDecision(e.target.value)}><option value="required">Required</option><option value="not_required">Not required</option></select></Field><Field label="Policy owner"><Input required value={policyOwner} onChange={(e) => setPolicyOwner(e.target.value)} /></Field><Field label="Policy reason"><Input required value={policyReason} onChange={(e) => setPolicyReason(e.target.value)} /></Field><Button type="submit" disabled={actorRole !== "admin" || isPending(FORM_KEYS.screeningPolicy)} aria-busy={isPending(FORM_KEYS.screeningPolicy)} variant="outline">Save registered/unregistered screening policy</Button>{message(FORM_KEYS.screeningPolicy)}
          </form>
          ...
          <form className="space-y-3 border-t pt-4" onSubmit={(e) => { e.preventDefault(); void call(FORM_KEYS.competenceRequirement, "cmd_admin_create_competence_requirement", { p_organisation_id: organisationId, p_role_version_id: role, p_support_category: supportCategory, p_evidence_type: requirementEvidenceType, p_requirement_state: requirementState, p_assessment_method: assessmentMethod, p_review_owner: requirementOwner, p_effective_from: effectiveFrom, p_effective_until: effectiveUntil, p_payload: { source: "admin-readiness" } }); }}><Field label="Competence evidence type"><Input required value={requirementEvidenceType} onChange={(e) => setRequirementEvidenceType(e.target.value)} /></Field><Field label="Requirement state"><select className={inputBase} value={requirementState} onChange={(e) => setRequirementState(e.target.value)}><option value="required">Required</option><option value="not_required">Not required</option></select></Field><Field label="Assessment method"><Input required value={assessmentMethod} onChange={(e) => setAssessmentMethod(e.target.value)} /></Field><Field label="Requirement owner"><Input required value={requirementOwner} onChange={(e) => setRequirementOwner(e.target.value)} /></Field><Button type="submit" disabled={actorRole !== "admin" || isPending(FORM_KEYS.competenceRequirement)} aria-busy={isPending(FORM_KEYS.competenceRequirement)}>Define competence requirement</Button>{message(FORM_KEYS.competenceRequirement)}</form>
          <div className="space-y-2 border-t pt-4 text-sm"><strong>Recorded readiness evidence</strong>{(data.screeningPolicies ?? []).map((entry) => <p key={String(entry.id)}>Policy · {roleLabel(String(entry.role_version_id))} · {String(entry.registration_state)} / {String(entry.decision)} · owner {String(entry.decision_owner)} · reason {String(entry.decision_reason)} · window {String(entry.effective_from)}–{String(entry.effective_until ?? "open")}</p>)}{(data.screeningVerifications ?? []).map((entry) => <p key={String(entry.id)}>Verification · worker {workerLabel(String(entry.worker_membership_id))} · role {roleLabel(String(entry.role_version_id))} · record {String(entry.application_or_check_reference)} · source {String(entry.source_checked)} · verifier {String(entry.verifier_name)} at {String(entry.verified_at)} · {String(entry.clearance_status)} · window {String(entry.effective_from)}–{String(entry.effective_until ?? "open")} · Clearance expires {String(entry.clearance_expires_at ?? "open")} · adverse: {([entry.interim_bar && "bar",entry.suspension && "suspension",entry.exclusion && "exclusion",entry.revocation && "revocation"].filter(Boolean).join(", ") || "none")}</p>)}{(data.screeningPathways ?? []).map((entry) => <p key={String(entry.id)}>Pathway · worker {workerLabel(String(entry.worker_membership_id))} · role {roleLabel(String(entry.role_version_id))} · {String(entry.pathway).replaceAll("_", " ")} · {String(entry.jurisdiction)} · placement record {String(entry.application_placement_contract_reference)} · supervisor {workerLabel(String(entry.supervisor_membership_id))} / clearance {String(entry.supervisor_clearance_reference)} · risk plan {String(entry.risk_management_plan_reference)} · window {String(entry.effective_from)}–{String(entry.effective_until ?? "open")} · Pathway valid from {String(entry.pathway_start)} · Pathway valid to {String(entry.pathway_end ?? "open")}</p>)}{(data.competenceRequirements ?? []).map((entry) => <p key={String(entry.id)}>Requirement · role {roleLabel(String(entry.role_version_id))} · {String(entry.evidence_type)} / {String(entry.requirement_state)} · {String(entry.support_category)} · method {String(entry.assessment_method)} · owner {String(entry.review_owner)} · window {String(entry.effective_from)}–{String(entry.effective_until ?? "open")}</p>)}{(data.competenceEvidence ?? []).map((entry) => <p key={String(entry.id)}>Evidence · worker {workerLabel(String(entry.worker_membership_id))} · requirement {requirementLabel(String(entry.requirement_id))} · record {String(entry.evidence_reference)} · issuer {String(entry.issuer)} · verifier {String(entry.verifier_name)} · {String(entry.assessed_state)}{entry.limitation ? ` · limitation ${String(entry.limitation)}` : ""} · expires {String(entry.expires_at ?? "open")} · window {String(entry.effective_from)}–{String(entry.effective_until ?? "open")}</p>)}</div>
        </CardContent></Card>
        <Card><CardHeader><CardTitle>6–10. Context, shift snapshot and acknowledgement</CardTitle><CardDescription>Only active, reviewed, current contexts schedule. Snapshot fields are immutable; acknowledgement is provider-recorded and never proof of participant authentication, consent or payment.</CardDescription></CardHeader><CardContent className="space-y-4">
          ...
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); void call(FORM_KEYS.contextState, "cmd_admin_update_service_context_state", { p_organisation_id: organisationId, p_context_id: lifecycleContextId, p_lifecycle_state: contextState, p_reviewer_profile_id: lifecycleReviewer || null, p_role_version_id: lifecycleRole || null, p_jurisdiction: lifecycleJurisdiction, p_reason: "Admin readiness review", p_payload: { source: "admin-readiness" } }); }}>
            <Field label="Service context"><select required className={inputBase} value={lifecycleContextId} onChange={(e) => chooseLifecycleContext(e.target.value)}><option value="">Choose context</option>{(data.serviceContexts ?? []).map((c) => <option key={String(c.id)} value={String(c.id)}>{String(c.goal_reference)} · {String(c.lifecycle_state)}</option>)}</select></Field>
            ...
            <Field label="Lifecycle reviewer"><select className={inputBase} value={lifecycleReviewer} onChange={(e) => setLifecycleReviewer(e.target.value)}><option value="">Choose reviewer</option>{officePeople.map((person) => <option key={String(person.profile_id)} value={String(person.profile_id)}>{identityLabel(String(person.profile_id))} · {String(person.role)}</option>)}</select></Field><Field label="Lifecycle role"><select className={inputBase} value={lifecycleRole} onChange={(e) => setLifecycleRole(e.target.value)}><option value="">Choose role</option>{(data.roles ?? []).map((entry) => <option key={String(entry.id)} value={String(entry.id)}>{String(entry.title)}</option>)}</select></Field><Field label="Lifecycle jurisdiction"><Input value={lifecycleJurisdiction} onChange={(e) => setLifecycleJurisdiction(e.target.value)} /></Field><p className="text-xs text-muted-foreground">Owner: {identityLabel(String(selectedLifecycleContext?.owner_profile_id ?? ""))}; current reviewer: {identityLabel(String(selectedLifecycleContext?.reviewer_profile_id ?? ""))}; current role: {String(data.roles?.find((entry) => String(entry.id) === String(selectedLifecycleContext?.role_version_id))?.title ?? "Not set")}; current jurisdiction: {String(selectedLifecycleContext?.jurisdiction ?? "Not set")}</p><Button type="submit" disabled={actorRole !== "admin" || isPending(FORM_KEYS.contextState)} aria-busy={isPending(FORM_KEYS.contextState)}>Save context lifecycle</Button>{message(FORM_KEYS.contextState)}
          </form>
          <form className="space-y-3 border-t pt-4" onSubmit={(e) => { e.preventDefault(); void checkReadiness(); }}><Field label="Readiness worker"><select required className={inputBase} value={worker} onChange={(e) => setWorker(e.target.value)}><option value="">Choose worker</option>{workers.map((entry) => <option key={String(entry.membership_id)} value={String(entry.membership_id)}>{String(entry.full_name ?? "Worker")}</option>)}</select></Field><Field label="Readiness context"><select required className={inputBase} value={readinessContextId} onChange={(e) => setReadinessContextId(e.target.value)}><option value="">Choose context</option>{(data.serviceContexts ?? []).map((entry) => <option key={String(entry.id)} value={String(entry.id)}>{participantLabel(String(entry.participant_id))} · {String(entry.goal_reference)} · {String(entry.lifecycle_state)}</option>)}</select></Field><Field label="Readiness start"><Input required type="datetime-local" value={scheduledStart} onChange={(e) => setScheduledStart(e.target.value)} /></Field><Field label="Readiness end"><Input required type="datetime-local" value={scheduledEnd} onChange={(e) => setScheduledEnd(e.target.value)} /></Field><Button type="submit" disabled={readinessPending} aria-busy={readinessPending}>Check readiness</Button></form>
          <div role="status" className="rounded-md border bg-muted/20 p-3 text-sm"><strong>Readiness result:</strong> {!readinessIsCurrent && readinessResult ? " Inputs changed — run readiness again before creating a service record." : readinessError ? ` Unable to check — ${readinessError}` : readinessResult?.ready === true ? ` Ready. Role, jurisdiction, catalogue and screening source are bound for the full interval.` : readinessResult ? ` Not ready — ${String(readinessResult.reason).replaceAll("_", " ")}. ${readinessRecovery[String(readinessResult.reason)] ?? "Review the provider readiness records above."}` : "Choose a worker, context and interval, then check the server-derived result."}</div>
          <form className="space-y-3 border-t pt-4" onSubmit={(e) => { e.preventDefault(); const participantId = String(selectedReadinessContext?.participant_id ?? ""); void call(FORM_KEYS.readinessShift, "cmd_admin_create_service_ready_shift", { p_organisation_id: organisationId, p_participant_id: participantId, p_worker_membership: worker, p_service_context_id: readinessContextId, p_scheduled_start: new Date(scheduledStart).toISOString(), p_scheduled_end: new Date(scheduledEnd).toISOString(), p_reason: "Provider readiness journey", p_payload: { source: "admin-readiness" } }); }}><Button type="submit" disabled={!readinessIsCurrent || readinessResult?.ready !== true || isPending(FORM_KEYS.readinessShift)} aria-busy={isPending(FORM_KEYS.readinessShift)}>Create service-ready shift</Button>{message(FORM_KEYS.readinessShift)}</form>
          ...
          <div className="space-y-2 rounded border p-3 text-sm"><strong>Acknowledgement ledger</strong>{selectedLedger.length === 0 ? <p>No provider-recorded acknowledgement events for this service record.</p> : selectedLedger.map((event) => <div key={String(event.id)} className="rounded bg-muted/30 p-2"><span>Record {String(event.id)} · {String(event.event_class)} · {String(event.event_type).replaceAll("_", " ")}</span>{event.current_leaf ? <strong> · current outcome</strong> : null}{event.review_only ? <strong> · preserved for evidence review</strong> : null}<p>Recorded by: {identityLabel(String(event.recorder_profile_id ?? ""))} · {String(event.source_channel)}; not participant-authenticated · {new Date(String(event.occurred_at)).toLocaleString("en-AU")}</p><p>Reported signer: {identityLabel(String(event.reported_signer_profile_id ?? ""))} · authority {String(event.authority_type ?? "attempt recorder only")} · source {String(event.authority_source_type ?? "not applicable")} / {String(event.authority_source_id ?? "no source record")} · method {String(event.method ?? "not applicable")}</p><p>Evidence: {String(event.external_evidence_reference ?? "none")} · supersedes {String(event.supersedes_event_id ?? "none (root/attempt)")}</p>{event.reason ? <p>Reason: {String(event.reason)}</p> : null}</div>)}</div>
        </CardContent></Card>
      </div>
    </div>
  );
