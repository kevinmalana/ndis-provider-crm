...
function Readiness({
  ...
  const currentReadinessFingerprint = JSON.stringify([worker, context, scheduledStart, scheduledEnd]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card><CardHeader><CardTitle>6–10. Context, shift snapshot and acknowledgement</CardTitle><CardDescription>Only active, reviewed, current contexts schedule. Snapshot fields are immutable; acknowledgement is provider-recorded and never proof of participant authentication, consent or payment.</CardDescription></CardHeader><CardContent className="space-y-4">
          ...
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); void call(FORM_KEYS.contextState, "cmd_admin_update_service_context_state", { p_organisation_id: organisationId, p_context_id: context, p_lifecycle_state: contextState, p_reviewer_profile_id: lifecycleReviewer || null, p_role_version_id: lifecycleRole || null, p_jurisdiction: lifecycleJurisdiction, p_reason: "Admin readiness review", p_payload: { source: "admin-readiness" } }); }}>
            <Field label="Service context"><select required className={inputBase} value={context} onChange={(e) => chooseLifecycleContext(e.target.value)}><option value="">Choose context</option>{(data.serviceContexts ?? []).map((c) => <option key={String(c.id)} value={String(c.id)}>{String(c.goal_reference)} · {String(c.lifecycle_state)}</option>)}</select></Field>
            ...
            <Field label="Lifecycle reviewer"><select className={inputBase} value={lifecycleReviewer} onChange={(e) => setLifecycleReviewer(e.target.value)}><option value="">Choose reviewer</option>{officePeople.map((person) => <option key={String(person.profile_id)} value={String(person.profile_id)}>{identityLabel(String(person.profile_id))} · {String(person.role)}</option>)}</select></Field><Field label="Lifecycle role"><select className={inputBase} value={lifecycleRole} onChange={(e) => setLifecycleRole(e.target.value)}><option value="">Choose role</option>{(data.roles ?? []).map((entry) => <option key={String(entry.id)} value={String(entry.id)}>{String(entry.title)}</option>)}</select></Field><Field label="Lifecycle jurisdiction"><Input value={lifecycleJurisdiction} onChange={(e) => setLifecycleJurisdiction(e.target.value)} /></Field><p className="text-xs text-muted-foreground">Owner: {identityLabel(String(selectedContext?.owner_profile_id ?? ""))}; current reviewer: {identityLabel(String(selectedContext?.reviewer_profile_id ?? ""))}; current role: {String(data.roles?.find((entry) => String(entry.id) === String(selectedContext?.role_version_id))?.title ?? "Not set")}; current jurisdiction: {String(selectedContext?.jurisdiction ?? "Not set")}</p><Button type="submit" disabled={actorRole !== "admin" || isPending(FORM_KEYS.contextState)} aria-busy={isPending(FORM_KEYS.contextState)}>Save context lifecycle</Button>{message(FORM_KEYS.contextState)}
          </form>
