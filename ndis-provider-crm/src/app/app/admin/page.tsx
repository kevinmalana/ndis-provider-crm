...
export default async function AdminPage() {
  ...
  const [participants, cards, memberships, shifts, assignments, authorities, grants, consents, availability, audit, identities, selfLinks, serviceContexts, providerScopes, capabilities, catalogues, catalogueItems, roles, screeningPolicies, screeningVerifications, screeningPathways, competenceRequirements, competenceEvidence, maskedIdentifiers, snapshots, ackLedger] = await Promise.all([
    ...
    supabase.from("worker_screening_verification_versions").select("id,worker_membership_id,role_version_id,source_checked,verifier_name,verified_at,application_or_check_reference,clearance_status,clearance_expires_at,interim_bar,suspension,exclusion,revocation,effective_from,effective_until,status").eq("organisation_id", organisationId).order("created_at", { ascending: false }),
    ...
    supabase.from("worker_competence_evidence_versions").select("id,worker_membership_id,requirement_id,evidence_type,issuer,evidence_reference,verifier_name,assessed_state,limitation,expires_at,effective_from,effective_until,status").eq("organisation_id", organisationId).order("created_at", { ascending: false }),
